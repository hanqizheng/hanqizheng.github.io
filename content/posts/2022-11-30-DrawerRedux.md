...
status: published
excerpt: 前些日子开发了一个 Drawer 组件的命令式使用方式，然后业务侧使用却发现了一个奇怪的问题。 起因 sugar 的 Drawer
  组件一直是这么使用的： 这么写本身没啥问题，但是随着业务的复杂性越来越高，可能某一个页面会用到 2-3 个甚至更多个 Drawer。 如果每一个
  Drawer 都需要去维护一个 state
---
