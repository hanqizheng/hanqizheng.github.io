...
status: published
excerpt: 新的需求是要用`Class Component`来写，说实话高举`hooks`真香旗帜的我已经有很长一段时间没有写过`Class
  Component`了。 React v16.3之后有几个生命周期被标记为`unsafe`，还添加了两个新的生命周期函数。
  为什么有的生命周期被标记为了`UNSAFE`? 新的生命周期又与之
---
